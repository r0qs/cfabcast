  ...
  def configuration(config: ClusterConfiguration): Receive = {
    ...
    case msg: DeliveredVMap =>
      ...
      if(vmap == None)
        ...
        log.info("Received vmap from {} with [{}] values.", sender, vmap.filter{ case (_, v) => v != Nil }.size)
        servers.foreach( server => { 
          log.debug("Sending response to server: {}", server)
          vmap.foreach({case (_ , value) =>
            value match {
              case values: Value =>
                log.info("NODE:{} - SENDING DELIVERED [{}] values, size: {} bytes", nodeId, values.size, values.sizeInBytes)
                val v = values.value
                if (v.nonEmpty){
                  v.foreach(o => o match { 
                    case Some(response) => 
