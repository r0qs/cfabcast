...
case class Msg1B(a: ActorRef, rnd: Long, vrnd: Long, vval: CStructType) extends Message

// Message sent by coordinator to all proposers and acceptors.
case class Msg2Start(rnd: Long, cval: CStructType) extends Message

// Message sent by coordinator c to all proposers.
case class Msg2Prepare(c: ActorRef, rnd: Long, value: CStructType) extends Message

// Message sent by collision-fast proposer cfp to all acceptors and others collision-fast proposers on the same round rnd.
case class Msg2A(cfp: ActorRef, rnd: Long, value: CStructType) extends Message

// Message sent by acceptor a to all learners.
case class Msg2B(a: ActorRef, rnd: Long, value: CStructType) extends Message
